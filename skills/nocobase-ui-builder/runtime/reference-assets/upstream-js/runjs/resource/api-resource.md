...
> Local skill note: use `APIResource` mainly for external `http/https` endpoints or custom relative URLs. For NocoBase collection data under this skill, prefer `ctx.initResource(...) + ctx.resource` or `ctx.makeResource(...)` with the appropriate collection resource type instead of `users:list`-style shorthand URLs.
...
- **Resource Style**: Product runtime supports NocoBase resource shorthand such as `users:list` or `posts:get`, which will be concatenated with the `baseURL`. Under this skill, keep that as reference-only behavior and prefer dedicated resource helpers for final NocoBase data access.
...
res.setURL('https://jsonplaceholder.typicode.com/users');
res.setRequestParameters({ _limit: 10 });
await res.refresh();
...
```js
const res = ctx.makeResource('APIResource');
res.setURL('/api/custom/stats');
res.setRequestParameters({ period: 'week' });
await res.refresh();
